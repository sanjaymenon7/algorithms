...
public class EvaluateDeluxe {
	//Modified Code from the site http://algs4.cs.princeton.edu/13stacks/EvaluateDeluxe.java.html
		...
		for (int n = 0; n < t; n++) {
			...
			for (int i = 0; i < eqnStr.length(); i++) {
				...
				if (Character.isDigit(character)) {
					int k = 0;
					while ((i < eqnStr.length())
							&& Character.isDigit(eqnStr.charAt(i))) {
				...
				while (true) {
					...
					if (ops.isEmpty() || s.equals("(")
							|| (precedence.get(s) > precedence.get(ops.peek()))) {
						ops.push(s);
						break;
					}
					...
					// evaluate operator and two operands and push result onto
					// value stack
					else {
						long val2 = vals.pop();
						long val1 = vals.pop();
			...
			// finished parsing string - evaluate operator and operands
			// remaining on two stacks
			while (!ops.isEmpty()) {
				String op = ops.pop();
				long val2 = vals.pop();
				long val1 = vals.pop();
			...
			System.out.println("Case #"+(n+1)+": "+vals.pop());
			assert vals.isEmpty();
			assert ops.isEmpty();
